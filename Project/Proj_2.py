...
class App(tk.Tk):
    def __init__(self):
        ...
        # canvas1 = tk.Canvas(root, height=HEIGHT, width=WIDTH, bg="yellow", alpha=.5)
        # canvas1.config(bg='')
        # root.wm_attributes("-transparentcolor", "yellow")


        # filename = tk.PhotoImage(file = "imageName.png")
        ...
        # frame1 = tk.Frame(root, bg="#f0f0f0", bd=5)
        # frame1.place(x=0, y=100, relwidth=1, height=100, anchor='n')
        ...
        def my_line():
            label.bind('<Button-1>', new_coord1)
            label.bind('<B1-Motion>',cre_line)
            label.bind('<ButtonRelease-1>', new_coord_line)
